...
func TestClient_ProtocolInfo(t *testing.T) {
	...
	tests := map[string]struct {
		platform     string
		protocolType cosi.ObjectProtocol_Type
	}{
		"s3 platform": {
			platform:     "s3",
			protocolType: cosi.ObjectProtocol_S3,
		},
		"azure platform": {
			platform:     "azure",
			protocolType: cosi.ObjectProtocol_AZURE,
		},
	}
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			assert.Equal(t, tc.protocolType, protocol.GetType())
		})
	}
}
